...
export default function PromotedBusinessesCarousel() {
  ...
  return (
    <section className="py-8 relative overflow-hidden group/carousel">
      ...
      <div className="container mx-auto max-w-6xl px-0 sm:px-4 relative">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 px-4 sm:px-0">
          <h2 className="text-xl md:text-2xl font-semibold text-foreground flex items-center gap-2 mb-4 md:mb-0 relative">
        ...
        >
          <CarouselContent className="-ml-2 md:-ml-3">
            {carouselBusinesses.map((business, index) => (
              <CarouselItem
                key={business.id}
                className="basis-[30%] sm:basis-[22%] md:basis-[18%] lg:basis-[14%] xl:basis-[12%] pl-2 md:pl-3"
              >
                <a
                  href={`/business/${business.id}`}
                  className="block group text-center transition-all duration-300 ease-out hover:scale-[1.03] hover:-translate-y-1"
                >
                  <div className="overflow-hidden rounded-xl aspect-square relative shadow-md mb-2 
                  ...
                  >
                    ...
                    <div className="absolute top-1 right-1 animate-orbit-small">
                      <Badge
                        className="bg-primary/90 hover:bg-primary text-primary-foreground text-[8px] md:text-[10px] px-1.5 py-0.5 rounded-full 
                          shadow-md shadow-primary/20 border-none backdrop-blur-sm"
                      >
                        ★ DESTACADO
                      </Badge>
                    </div>
                  ...
                  <div className="flex items-center justify-center text-[10px] text-muted-foreground gap-1">
                    <div className="flex items-center gap-0.5">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-yellow-500 fill-current" viewBox="0 0 24 24">
          ...
          <CarouselPrevious className="absolute -left-3 sm:left-0 top-1/2 -translate-y-1/2 z-20 opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-300 bg-background/80 hover:bg-background border-primary/20 h-8 w-8" />
          <CarouselNext className="absolute -right-3 sm:right-0 top-1/2 -translate-y-1/2 z-20 opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-300 bg-background/80 hover:bg-background border-primary/20 h-8 w-8" />
        </Carousel>
      </div>
    </section>
  );
